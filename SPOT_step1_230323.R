...
#  ymax <- 212
  ...
  canopy <- raster(x="/work/u9197633/SPOT_CSRSR/canopyheight_model/lidar_chikueiwang_20m/20m_CHM_resample6m.tif")
  # Global Land Analysis and Discover team (GLAD) at the University of Maryland's CHM 25m downsampled to 6m
  # canopy <- raster(x="/home/u3176872/LULCC/sourcefiles/canopyheight_model/canopyheight_25m/TW_CHM_resample6m.tif")
  #set crs as TWD97
  crs(canopy) <- "+proj=tmerc +lat_0=0 +lon_0=121 +k=0.9999 +x_0=250000 +y_0=0 +ellps=GRS80 +units=m +no_defs"
  
  #import population data raster for future usage
  population <- raster(x="/home/u3176872/LULCC/sourcefiles/population_ytlin/population_6m.tif")
  #set crs as TWD97
  crs(population) <- "+proj=tmerc +lat_0=0 +lon_0=121 +k=0.9999 +x_0=250000 +y_0=0 +ellps=GRS80 +units=m +no_defs"
  ...
  for (yr in str_yr:end_yr){ #doesn't iterate yr 2018 because when iterating 2017, 2018 will be counted in
    ...
    for (aoi in 1:length(allaoi)){
      ...
      directory <- paste("/work/u9197633/SPOT_CSRSR/grid_box/",yr,"/",sep="")
    ...
    for (aoi in 1:length(allaoi)){
      ...
      if (yr==2017){ #separate if statement for years 2017 2018 to bind two year's images to classify
        ...
        directory <- c("/work/u9197633/SPOT_CSRSR/grid_box/2017/")
        #create list of SPOT image under directory that satisfy condition
        aoi_images <- dir(path = directory, pattern = "SPOT", all.files = FALSE,
                          full.names = FALSE, recursive = FALSE, ignore.case = FALSE, include.dirs = FALSE)
        #use all images that satisfies aoi location
        ...
        directory <- c("/work/u9197633/SPOT_CSRSR/grid_box/2018/")
        #create list of SPOT image under directory that satisfy condition
        ...
        for (item in 1:length(aoi_images)){
          if (substr(aoi_images[item],start=7,stop=10)==2017){
            testlayer <- raster(x=paste("/work/u9197633/SPOT_CSRSR/grid_box/2017/",aoi_images[item],"/",aoi_images[item],".1.bsq.ers",sep=""))
          }else if (substr(aoi_images[item],start=7,stop=10)==2018){
            testlayer <- raster(x=paste("/work/u9197633/SPOT_CSRSR/grid_box/2018/",aoi_images[item],"/",aoi_images[item],".1.bsq.ers",sep=""))
          }
          if (item==1){
        ...
        for (item in 1:length(aoi_images)){
          if (substr(aoi_images[item],start=7,stop=10)==2017){
            band <- raster(x=paste("/work/u9197633/SPOT_CSRSR/grid_box/2017/",aoi_images[item],"/",aoi_images[item],".1.bsq.ers",sep=""))
          }else if (substr(aoi_images[item],start=7,stop=10)==2018){
            band <- raster(x=paste("/work/u9197633/SPOT_CSRSR/grid_box/2018/",aoi_images[item],"/",aoi_images[item],".1.bsq.ers",sep=""))
          }
        ...
        for (month in 1:12){ #12 months
          ...
          if (length(subset_aoiimages)==0){
            ...
            for (image in 1:length(subset_aoiimages)){
              if (substr(subset_aoiimages[image],start=7,stop=10)==2017){
                ...
                for (band in 1:4){
                  ...
                  assign(paste("band",band,sep=""),  raster(x=paste("/work/u9197633/SPOT_CSRSR/grid_box/2017/",
                ...
                for (band in 1:4){
                  #assign SPOT image band digital count values to variables band1-4
                  assign(paste("band",band,sep=""),  raster(x=paste("/work/u9197633/SPOT_CSRSR/grid_box/2018/",
                                                            subset_aoiimages[image],"/",subset_aoiimages[image],".",band,".bsq.ers",sep="")))
                } #end of band-loop
              }
            ...
            #crop population raster based on the extent of aoi, therefore extent of medianNDVI_ofmonth (or medianNIR_ofmonth) (classified result) raster
            population_crop <- crop(population, medianNDVI_ofmonth)
            #set extent of cropped population raster just like medianNDVI_ofmonth (or medianNIR_ofmonth) raster
            extent(population_crop) <- extent(medianNDVI_ofmonth)
            #make sure the raster size is the same by uisng the project function for the cropped population raster to  match the dimension of the medianNDVI_ofmonth
            population_crop <- projectRaster(population_crop,medianNDVI_ofmonth,method = 'bilinear')
            #convert population_crop to dataframe and add to dataframe
            population_df <- as.data.frame(population_crop)
            rasterallpoints["population"] <- population_df
            
            #crop canopy raster based on the extent of aoi, therefore extent of medianNDVI_ofmonth (or medianNIR_ofmonth) (classified result) raster
            canopy_crop <- crop(canopy, medianNDVI_ofmonth)
            #set extent of cropped canopy raster just like medianNDVI_ofmonth (or medianNIR_ofmonth) raster
            extent(canopy_crop) <- extent(medianNDVI_ofmonth)
            #make sure the raster size is the same by uisng the project function for the cropped population raster to  match the dimension of the medianNDVI_ofmonth
            canopy_crop <- projectRaster(canopy_crop,medianNDVI_ofmonth,method = 'bilinear')
            #convert canopy_crop to dataframe and add to dataframe
            canopy_df <- as.data.frame(canopy_crop)
            rasterallpoints["canopy"] <- canopy_df
      ...
      allpoints["class"] <- NA
      #thresholds
      #only apply population data to this branch of water
      allpoints$class[allpoints$max_N<=0.03 & allpoints$population<=2000] <- 3
      allpoints$class[allpoints$max_N<=0.03 & allpoints$population>2000] <- 2
      ...
      #only apply population and canopy height data to water
      allpoints$class[allpoints$max_N>0.03 & allpoints$max_N<=0.19 & allpoints$median_NIR<=400 & allpoints$population>2000 & allpoints$canopy>1] <- 2
      allpoints$class[allpoints$max_N>0.03 & allpoints$max_N<=0.19 & allpoints$median_NIR<=400 & allpoints$population>2000 & allpoints$canopy<=1] <- 3
      allpoints$class[allpoints$max_N>0.03 & allpoints$max_N<=0.19 & allpoints$median_NIR<=400 & allpoints$population<=2000] <- 3
      ...
      #--------------------#canopy height model filters FA mixups#--------------------#
      #set crs as TWD97
      ...
      #before doing raster stack function, need to  make sure the raster size is the same by uisng the project function 
      #for the canopy raster to  match the dimension of the  outraster data (added by YY Chen)
      canopyraster <- projectRaster(canopyraster,outputraster,method = 'bilinear')

      #stack classified result and cropped canopy raster
      comparestack <- stack(outputraster, canopyraster)
      
 
      comparestack <- as.data.frame(comparestack)
      #rename comparestack df columns to class (classification land type) and height (canopy height)
      colnames(comparestack) <- c("class", "height")
      ...
      #rearrange dataframe columns order to index, class, height
      comparestack <- comparestack[,c(3,1,2)]
      ...
      #check canopy height <=0.5 for the shadow effect of high buildings reset the water body as builtup (YY Chen for testing the code) 
      #comparestack["class"][(comparestack["class"]==3)&(comparestack["height"]>=1.5)] <- 2
      #--------------------#
      
      #output dataframe as raster
      #duplicate medianNDVI_ofmonth raster to copy dimension and extent
      outputraster<- medianNDVI_ofmonth
      values(outputraster) <- NA
      ...
      image_path <- c(paste("/home/u3176872/LULCC/plots/classificationresults/taiwan/",yr,"/",sep=""))
      ...
      if (yr==2017){
        writeRaster(outputraster,paste(image_path,"2017and2018_",allaoi[aoi],"_tree18_CHM20m.tif",sep=""),overwrite=TRUE)

        png(paste(image_path,"2017and2018_",allaoi[aoi],"_tree18_CHM20m.png",sep=""),
             width = 1080, height = 1080, units = "px")
      }else{
        writeRaster(outputraster,paste(image_path,yr,"_",allaoi[aoi],"_tree18_CHM20m.tif",sep=""),overwrite=TRUE)
       
        png(paste(image_path,yr,"_",allaoi[aoi],"_tree18_CHM20m.png",sep=""),
            width = 1080, height = 1080, units = "px")
      }
      plot(outputraster,
           col=colors)
      ...
      print(paste("please chech the image file:", image_path,yr,"_",allaoi[aoi],".png",sep="") )
      # print current time
      print(Sys.time())
