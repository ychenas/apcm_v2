...
#  ymax <- 212
  ...
  for (yr in str_yr:end_yr){ #doesn't iterate yr 2018 because when iterating 2017, 2018 will be counted in
    ...
    for (aoi in 1:length(allaoi)){
      ...
      if (yr==2017){
        writeRaster(outputraster,paste(image_path,"2017and2018_",allaoi[aoi],"_tree18_CHM25m.tif",sep=""),overwrite=TRUE)

        png(paste(image_path,"2017and2018_",allaoi[aoi],"_tree18_CHM25m.png",sep=""),
             width = 1080, height = 1080, units = "px")
      }else{
        writeRaster(outputraster,paste(image_path,yr,"_",allaoi[aoi],"_tree18_CHM25m.tif",sep=""),overwrite=TRUE)
       
        png(paste(image_path,yr,"_",allaoi[aoi],"_tree18_CHM25m.png",sep=""),
            width = 1080, height = 1080, units = "px")
      }
      plot(outputraster,
           col=colors)
