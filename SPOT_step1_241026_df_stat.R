  ...
  for (yr in str_yr:end_yr){ #doesn't iterate yr 2018 because when iterating 2017, 2018 will be counted in
   ...
   #aoi loop start 
	...
	print(paste("aoi_images after checking:"))
      ...
      #call the upsacle function HERE  
      #convert XID YID to index
      ...
      # find the imesh id 
      imesh = which(mesh.12km$XID_YID== paste('(', substr(allaoi[aoi],start=1,stop=3),",",substr(allaoi[aoi],start=5,stop=7),')',sep="") )
