...
#  ymax <- 212
  ...
  for (yr in str_yr:end_yr){ #doesn't iterate yr 2018 because when iterating 2017, 2018 will be counted in
    ...
    for (aoi in 1:length(allaoi)){
      ...
      # allpoints$class[allpoints$max_N>0.03 & allpoints$max_N<=0.19 & allpoints$median_NIR<=400 & allpoints$population>2000] <- 2 #add threshold of population data: if population > 2000: builtup, <=2000: water
      # allpoints$class[allpoints$max_N>0.03 & allpoints$max_N<=0.19 & allpoints$median_NIR<=400 & allpoints$population<=2000] <- 3 #add threshold of population data
      # allpoints$class[allpoints$max_N>0.03 & allpoints$max_N<=0.19 & allpoints$median_NIR>400 & allpoints$population>2000] <- 2 #add threshold of population data
      # allpoints$class[allpoints$max_N>0.03 & allpoints$max_N<=0.19 & allpoints$median_NIR>400 & allpoints$population<=2000] <- 3 #add threshold of population data
      #only apply population data to water
      allpoints$class[allpoints$max_N>0.03 & allpoints$max_N<=0.19 & allpoints$median_NIR<=400 & allpoints$population>2000] <- 2
      ...
      if (yr==2017){
        writeRaster(outputraster,paste(image_path,"2017and2018_",allaoi[aoi],"_tree15.tif",sep=""),overwrite=TRUE)

        png(paste(image_path,"2017and2018_",allaoi[aoi],"_tree15.png",sep=""),
             width = 1080, height = 1080, units = "px")
      }else{
        writeRaster(outputraster,paste(image_path,yr,"_",allaoi[aoi],"_tree15.tif",sep=""),overwrite=TRUE)
       
        png(paste(image_path,yr,"_",allaoi[aoi],"_tree15.png",sep=""),
            width = 1080, height = 1080, units = "px")
      }
      plot(outputraster,
           col=colors)
